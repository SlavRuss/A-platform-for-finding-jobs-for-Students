...
from config.settings import TABLE_RECOMMENDATIONS, TABLE_STUDENTS, TABLE_VACANCIES, TABLE_VACANCY_SKILLS, TABLE_AREAS, TABLE_COMPANIES, TABLE_SKILLS
...
class RecommendationCRUD:
    ...
    def clear_all_recommendations(self):
        query = f"DELETE FROM {TABLE_RECOMMENDATIONS}"
        self.db.execute_query(query)
        print("Все рекомендации удалены")
    
    def clear_student_recommendations(self, student_id):
        query = f"DELETE FROM {TABLE_RECOMMENDATIONS} WHERE student_id = %s"
        self.db.execute_query(query, (student_id,))
        print(f"Рекомендации для студента {student_id} удалены")
    
    def count_recommendations(self):
        query = f"SELECT COUNT(*) FROM {TABLE_RECOMMENDATIONS}"
        result = self.db.execute_query(query, fetch=True)
        return result[0][0] if result else 0
    
    def count_student_recommendations(self, student_id):
        query = f"SELECT COUNT(*) FROM {TABLE_RECOMMENDATIONS} WHERE student_id = %s"
        result = self.db.execute_query(query, (student_id,), fetch=True)
        return result[0][0] if result else 0
    ...
    def print_table_structure(self):
        ...
        WHERE table_schema = 'msod4' AND table_name = 'recommendations'
